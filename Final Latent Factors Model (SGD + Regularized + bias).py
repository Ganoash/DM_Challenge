...
user_bias = [
    np.mean(user_ratings[user_ratings != 0]) for user_ratings in tmp_R.T
] - overall_bias

movie_bias = [
    np.mean(movie_ratings[movie_ratings != 0]) for movie_ratings in tmp_R
] - overall_bias
...
R_pred_pre = (
    overall_bias
    + np.tile(user_bias, (num_movies, 1))
    + np.tile(movie_bias, (num_users, 1)).T
    + np.matmul(Q, P)
)
...
for epoch in range(EPOCHS):
    ...
    for userID, movieID, rating in ratings:
        q_update = LEARNING_RATE * (
            curr_error[movieID, userID] * P[:, userID] - LAMBDA * Q[movieID, :]
        )
        Q[movieID, :] = Q[movieID, :] + q_update

        p_update = LEARNING_RATE * (
            curr_error[movieID, userID] * Q[movieID, :] - LAMBDA * P[:, userID]
        )
        P[:, userID] = P[:, userID] + p_update

        movie_bias_update = LEARNING_RATE * (
            curr_error[movieID, userID] - LAMBDA_bias * movie_bias[movieID]
        )
        movie_bias[movieID] = movie_bias[movieID] + movie_bias_update

        user_bias_update = LEARNING_RATE * (
            curr_error[movieID, userID] - LAMBDA_bias * user_bias[userID]
        )
...
plt.plot(
    objective_sgd[:190, 0],
    objective_sgd[:190, 1],
    label="Latent Factor SGD + Regularization",
)
plt.plot(
    objective_bias[:, 0],
    objective_bias[:, 1],
    label="Latent Factor SGD + Regularization + bias",
)
